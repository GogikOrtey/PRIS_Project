...
def Req():
    ...
    if a_1_MinTempInHome == 2:
        strRequare += "min_temperature >= " + str(a_1_input_MinTempInHome)
    ...
    # У меня есть как минимум 3 вопроса, которые задаются пользователю из дома, если он выбрал "На улице"

    # Цветные растения обычно не плодоносят
    # Убрать проверку на переносимость прямого света

    # Мне действительно нужно будет сделать авто-тесты, на все характеристики. Как минимум, для того что бы отловить ошибки


# Если после ввода какой-то харакетистики у нас получается пустой ответ, то мы его не указываем - ???
...
#                     a_1_input_MinTempInHome = c
#     else:
#         for d in range(15, 31):
#             a_2_input_AVGTempInRegion = d
#             for e in range(1, 5):
#                 a_2_1_AVGHum = e
#                 if a_2_1_AVGHum == 1:
...
#                 part_2()
#     else:
#         for d in range(15, 31, 4):
#             a_2_input_AVGTempInRegion = d
#             for e in range(1, 5):
#                 a_2_1_AVGHum = e
#                 if a_2_1_AVGHum == 1:
#                     for f in range(20, 91, 5):
#                         a_2_1_input_AVGHum = f
#                         part_2()
#                 else:
#                     part_2()
    ...
    if a_InHome == 1: # На улице
        for b in range(1, 3):
            ...
            if(a_1_MinTempInHome == 1): # Минимальная температура на улице
                for c in range(-15, 0, 4):
                    a_2_input_AVGTempInRegion = c    
                    part_2()
            else:
                part_2()    
    else:
        # for dd in range(15, 30, 4):
...
# for a in range(1, 3): 
#     a_InHome = a
#     if a_InHome == 2: # На улице
#         for b in range(1, 3):
#             a_1_MinTempInHome = b
#             if a_1_MinTempInHome == 2:
#                 for c99 in range(-20, 0, 4):
#                     a_1_input_MinTempInHome = c99
#                     part_2()
#     else: # В доме
#         for cc in range(15, 36, 4):
#             a_2_input_MinTempInHome = cc-5
#             a_2_input_MaxTempInHome = cc+7
...
for a in range(1, 3): # Упрощённый режим, без температуры и влажности
    a_InHome = a
    part_2()
...
# Зелёные растения чаще всего не плодоносные
# Если пользователь введёт эти 2 фактора, то нужно будет написать ему об этом,
# И вывести для него, например подборку только зелёных, или только плодоносных растений


# is_fruitful = 'true' - иногда вызывает пустые ответы
