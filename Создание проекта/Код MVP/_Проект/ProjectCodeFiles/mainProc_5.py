...
# Предупреждение: Пожалуйста, не заполняйте этот тест от балды. У нас не найдётся
# плодоносного растения, с серебрянными цветами, которое растёт в доме, при температуре 5 градусов, 
# и влажности 90%. А также стоит на окне и боится сильного света

# Если результатов нет, то выводить примерно это:
# Вот единственный цветок, который вам подойдёт: И картинка AmongUs с ножом
...
if a_InHome == 1:
    ...
    # a_2: Какая средняя температура в вашем доме?
    a_1_MinTempInHome = get_input("Какая средняя температура в вашем доме? ", 15, 30)
